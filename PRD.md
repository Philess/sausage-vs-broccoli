...
**Complexity Level**: Light Application (multiple features with basic state)
  - Game features include character movement, jumping mechanics, enemy AI, collision detection, score tracking, and game state management (playing, game over, win)
...
### Score & Game State
- **Functionality**: Track defeated enemies, display score, detect win/loss conditions
- **Purpose**: Provide goals, feedback, and game progression
- **Trigger**: Continuous during gameplay
- **Progression**: Game starts → Score at 0 → Defeat enemy → Score increases → All enemies defeated → Win screen OR Touch enemy from side/below → Game over screen
- **Success criteria**: Persistent score display, clear win/loss screens with restart option
...
- **Components**: 
  ...
  - Button (restart, customized with rounded corners and bounce hover effect)
  - Custom canvas component for game rendering
  - Badge (score display with pulsing animation on score increase)
- **Customizations**: 
